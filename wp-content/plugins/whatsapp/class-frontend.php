...
function wa_btn($content) {
	...
	if (
       ...
       ((is_home() || is_front_page()) && $options['homepage'] == 'on')) {
		$btn = '<!-- WhatsApp Share Button for WordPress: http://peadig.com/wordpress-plugins/whatsapp-share-button/ --><div class="wabtn_container"><a href="whatsapp://send?text='.get_the_title().' - '.get_permalink().$tracking.'" class="wabtn">WhatsApp</a></div>';
...
function wa_btn_shortcode($waatts) {
	...
	$btn = '<!-- WhatsApp Share Button for WordPress: http://peadig.com/wordpress-plugins/whatsapp-share-button/ --><div class="wabtn_container"><a href="whatsapp://send?text='.$title.' - '.$url.$tracking.'" class="wabtn">WhatsApp</a></div>';
	return $btn;
}
